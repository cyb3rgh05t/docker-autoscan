...
log = logging.getLogger("autoscan.targets.emby")


class EmbyTarget(BaseTarget):
    def __init__(self, cfg: dict[str, Any]) -> None:
        super().__init__(cfg, f"emby({cfg.get('url', '')})")
    ...
    async def scan(self, folder: str) -> None:
        ...
        if not matching:
            log.warning("Emby: no library found for path %s", folder)
            return
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            for lib in matching:
                ...
                resp.raise_for_status()
                log.info("Emby scan triggered: library=%s path=%s", lib["name"], folder)
